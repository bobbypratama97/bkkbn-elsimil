        ...
        <div class="d-flex flex-column-fluid">
            <div class="container-fluid">
                ...
                <div class="card card-custom gutter-b example example-compact">
                    ...
                    <form class="form" method="POST" action="{{ route('news.update', $data->id) }}" id="my-awesome-dropzone" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <input type="hidden" id="gambarExist" name="image" value="{{ $data->thumbnail }}">
...
<script>
    ...
    Dropzone.options.dropzone = {
        ...
        init: function() {
            ...
            var imagesList = [baseurl + "/uploads/artikel/ori/" + '{{ $data->thumbnail }}'];
            for (var i = 0; i < imagesList.length; i++) {
                ...
                fetch(imagesList[i])
                 .then(res => res.blob())
                 .then(blob => {
                    var file = new File([blob], name, blob);
            ...
            this.on("maxfilesexceeded", function(file) {
                bootbox.alert({
                    ...
                    centerVertical: true,
                    closeButton: false,
                    message: "Hanya bisa upload 1 gambar saja. Silahkan hapus gambar yang sudah ada terlebih dahulu.",
                    size: 'small'
                });
            ...
            this.on("thumbnail", function(file, dataUrl) {
                if (file.width > maxImageWidth || file.height > maxImageHeight) {
                    bootbox.alert({
                        ...
                        centerVertical: true,
                        closeButton: false,
                        message: "<p class='text-center'>Ukuran file melebihi <span class='text-danger font-weight-bolder'>lebar 500 pixel dan tinggi  500 pixel</span>. <br />Silahkan upload ulang gambar.</p>",
                        size: 'small'
                    });
                    this.removeFile(file);
            ...
            this.on("success", function(file, response) {
                ...
                var dialog = bootbox.dialog({
                    ...
                    centerVertical: true,
                    closeButton: false,
                    message: "<p class='text-center'><strong>" + json.message + "</p>",
                    buttons: {
                        ok: {
                            label: "Kembali ke List Artikel",
                            className: 'btn-info',
                            callback: function() {
