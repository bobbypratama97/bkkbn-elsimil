                ...
                <div class="card card-custom gutter-b">
                    <div class="card-header flex-wrap py-3">
                        <div class="card-title">
                            <h3 class="card-label">Kabupaten 
                            <span class="d-block text-muted pt-2 font-size-sm">Halaman ini menampilkan data kabupaten</span></h3>
                        </div>
                        @can('access', [\App\Kota::class, Auth::user()->role, 'upload'])
                        <div class="card-toolbar">
                            <div class="col text-left">
                                <a href="{{ route('admin.kota.template') }}" class="btn btn-warning">Download Template Kabupaten</a>
                            </div>
                            <a href="{{ route('admin.kota.upload') }}" class="btn btn-primary font-weight-bolder">
                                ...
                                </span>Upload Master Kota</a>
                        </div>
                        @endcan
                    </div>
                    <div class="card-body">
                        <form class="form-inline mb-7" method="GET" action="{{ route('admin.kota.index') }}">
                            <div class="form-group mr-3">
                                <label for="name">Cari : </label>
                                <input type="search" name="name" value="{{ (isset($name)) ? $name : ""}}"  class="form-control form-control-sm ml-3" placeholder="Provinsi, Kota" aria-controls="kt_datatable" _vkenabled="true">
                            </div>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="  white-space: nowrap;">
                                        @can('access', [\App\Kota::class, Auth::user()->role, 'edit'])
                                        <a href="{{ route('admin.kota.edit', $row->id) }}" class="btn btn-icon btn-sm btn-info" data-toggle="tooltip" data-placement="top" title="Ubah">
                                            <i class="flaticon2-edit"></i>
                                        </a>
                                        @endcan
                                        @can('access', [\App\Kota::class, Auth::user()->role, 'delete'])
                        ...
                        <div class="float-left">
                            @if (count($kota) > 1)
                                Menampilkan {{ ($paginate->currentPage() * 10) - 10 + 1 }} sampai {{ (($paginate->currentPage() * 10) - 10) + count($kota) }} dari {{ $paginate->total() }} data
                            @else
                            ...
                            bootbox.dialog({
                                ...
                                buttons: {
                                    ok: {
                                        ...
                                        className: 'btn-info',
                                        callback: function() {
                                            window.location.href = '{{ route('admin.kota.index') }}';
                                        }
                                    }
                                }
                            });
