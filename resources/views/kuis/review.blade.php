                    ...
                    <div class="card-header">
                        ...
                        <div class="card-toolbar">
                            <div class="example-tools justify-content-center">
                                ...
                                <a href="{{ route('admin.kuis.approve') }}" class="btn btn-danger">Kembali</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="card card-custom">
                            <div class="card-header h-auto py-4">
                                <div class="card-title">
                                    <i class="flaticon2-writing mr-3"></i> <h3 class="card-label">Catatan </h3>
                                </div>
                            </div>
                            <form method="POST" action="{{ route('admin.kuis.submitreview') }}" class="form">
                                @csrf
                                <input type="hidden" name="cid" value="{{ $kuis->id }}">
                                <div class="card-body py-4">
                                    <div class="form-group">
                                        <label>Catatan</label>
                                        <textarea class="form-control" rows="5" name="catatan">{{ (isset($log) && !empty($log->catatan)) ? $log->catatan : '' }}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label>Proses Approval</label>
                                        <select class="form-control" name="apv" required oninvalid="this.setCustomValidity('Proses approval harus diisi')" oninput="setCustomValidity('')">
                                            @foreach ($status as $key => $row)
                                            @if ($key != 'APV100')
                                            <option value="{{ $key }}" {{ $key == $kuis->apv ? 'selected' : '' }}>{{ $row }}</option>
                                            @endif
                                            @endforeach
                                        </select>
                                    </div>
