...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if ($role->role_id == '5') {
            ...
            $whereChats = " WHERE ch.responder_id = ".$auth->id." AND mb.provinsi_id = ".($auth->provinsi_id ?? 0)." AND mb.kabupaten_id = ".($auth->kabupaten_id ?? 0)." AND mb.kecamatan_id = ".($auth->kecamatan_id ?? 0)." AND mb.kelurahan_id = ".($auth->kelurahan_id ?? 0);
            $whereAllChats = " WHERE (ch.responder_id is null OR ch.responder_id = {$auth->id}) AND mb.provinsi_id = ".($auth->provinsi_id ?? 0) ." AND mb.kabupaten_id = ".($auth->kabupaten_id ?? 0) ." AND mb.kecamatan_id = ".($auth->kecamatan_id ?? 0)." AND mb.kelurahan_id = ".($auth->kelurahan_id ?? 0);
            $whereKuis = " WHERE kr.status = 1 AND kr.responder_id = {$auth->id} AND krc.id IS NULL AND mb.provinsi_id = ". ($auth->provinsi_id ?? 0) ." AND mb.kabupaten_id = ".($auth->kabupaten_id ?? 0) ." AND mb.kecamatan_id = ".($auth->kecamatan_id ?? 0)." AND mb.kelurahan_id = ".($auth->kelurahan_id ?? 0);
            $whereAllKuis = " WHERE kr.status = 1 AND krc.id IS NULL AND mb.provinsi_id = ". ($auth->provinsi_id ?? 0) ." AND mb.kabupaten_id = ". ($auth->kabupaten_id ?? 0) ." AND mb.kecamatan_id = ". ($auth->kecamatan_id ?? 0)." AND mb.kelurahan_id = ".($auth->kelurahan_id ?? 0);
            $whereUnmap = " WHERE md.member_id IS NULL AND mb.provinsi_id = ". ($auth->provinsi_id ?? 0) ." AND mb.kabupaten_id = ". ($auth->kabupaten_id ?? 0) ." AND mb.kecamatan_id = ". ($auth->kecamatan_id ?? 0)." AND mb.kelurahan_id = ".($auth->kelurahan_id ?? 0);
